...
from .serializers import (UserWithRecipesSerializer, TagSerializer,
                          IngredientSerializer, RecipeSerializer,
                          CreatRecipeSerializer, RecipeMinifiedSerializer)
from users.models import Subscription
from recipes.models import Tag, Ingredient, Recipe, Favorite, Shopping_cart, IngredientInRecipe, Amount
...
class CastomDjUserViewSet(UserViewSet):
    ...
    def subscriptions(self, request, *args, **kwargs):
        '''Мои подписки.
        '''
        follow = Subscription.objects.values_list('author',
                                                  flat=True
                                                  ).filter(
                                                    user=request.user
                                                    )
...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def shopping_cart(self, request, pk=None):
        ...
        sub_post = Shopping_cart.objects.filter(recipe=recipe_obj,
                                                user=user_obj,
                                                )
        if request.method == 'POST':
            if sub_post.exists():
                ...
                return Response(mess, status=status.HTTP_400_BAD_REQUEST)
            Shopping_cart.objects.create(recipe=recipe_obj,
                                         user=user_obj,
                                         )
